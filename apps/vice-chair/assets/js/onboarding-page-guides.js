(function(global){
  ...
  catalog.registerGuides({
    ...
    "page:training-catalog":page({
      key:"training-catalog",name:"培訓課程查詢",title:"依課名與期間找到培訓場次",
      vp:"查閱官方已公布課程，安排適合的日期；課表不代表報名或培訓完成。",
      steps:[
        s("search","查找課程","#searchForm","搜尋特定課程","輸入 MSP 下、一對一等課名，結果依日期排列；搜尋可搭配目前選擇的期間。"),
        s("period","查詢期間","#periods","本月、下月與全年","一鍵查看今年或明年全年；指定月份與自訂日期可以自由縮小範圍。"),
        s("source","課表來源",".sync-bar","核對最後成功更新時間","重新整理讀取資料庫最新課表；副主席可立即更新官方資料。同步失敗保留上次資料。"),
        s("changes","課程異動","#courseChanges","展開查看異動前後資訊","改期保留原日期；官方暫未列出時標示待確認，不當作已取消。")
      ]
    }),
