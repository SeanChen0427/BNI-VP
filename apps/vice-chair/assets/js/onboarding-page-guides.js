  ...
  catalog.registerGuides({
    "page:renewal-foundations":page({
      ...
      steps:[
        ...
        s("form-help","表單教學","#importLegacyFoundation","開啟表單後逐欄學習","進入新增或補登表單後，首次會逐一指出當前欄位、必填原因及儲存按鈕；之後可按「表單逐步教學」重看。切換計算期間會顯示不同欄位，重看時只教現在適用的內容，教學不會代填或保存。",{roles:VP}),
        ...
        s("action-help","逐項操作","#foundationList","展開後按「各項操作教學」","每項地基的「各期進度與操作」內都有教學入口，直接框出查看歷程、記錄、複製、修改、刪除或復原等目前可用按鈕。打開紀錄表單後，還能學本次動作、期別、回覆及保存邏輯。"),
        s("workshop","核對完成","#foundationList","工作坊由副主席逐期確認","核對實際參加日期與證據後，確認對應期間。單一期完成不代表往後各期完成；同一期多場，備註列出各場日期與內容。",{roles:VP}),
        s("remind","留下提醒","#foundationList","實際聯繫後再記錄已提醒","主責與陪同人員可複製提醒文案，實際聯繫後記錄日期、管道及回覆。複製文字或通知已讀不算已提醒；地基會繼續列管到確認結果。"),
    ...
    "page:settings":page({
      ...
      steps:[
        ...
        s("release","系統資訊","#releaseNotes","系統版本與更新","閱讀面向使用者的功能變更、影響與日期，不顯示敏感技術設定。")
      ]
    }),
    ...
    "page:course":{
      ...
      page:"交接與學習",
      roles:VP,
      intro:intros("交接與學習","這段只教你使用課程介面","制度與職務內容由課程本身說明；這份操作導覽只介紹目錄、進度、續看、切換及重設。"),
      steps:[
        s("menu","課程導覽","#open","開啟課程目錄","窄螢幕上展開左側章節；開合不改變進度。",{viewports:["mobile"]}),
        s("back","課程導覽",".dashboard-back","回工作台","返回工作總覽；課程進度會保存，並與系統操作導覽分開。"),
        s("progress","課程進度",'[data-guide-id="course.progress"]',"整體完成比例","完整進度卡會顯示百分比、進度條與已完成章數；按章節內完成按鈕後才計入。"),
        s("nav","課程進度","#nav","章節目錄","查看標題與完成狀態，並切換可用章節；這不是工作台功能選單。"),
        s("welcome","開始學習","#welcome","課程首頁","顯示範圍、章節和預估時間；新使用者開始第一章，有進度者可續看。"),
        s("lesson","章節閱讀","#lessonScreen","教材、章節進度與重點","主教材才是制度教學；本操作導覽不重複制度內容。"),
        s("footer","章節閱讀","#footer","上一章與完成並繼續","完成並繼續會更新課程進度；回上一章不取消既有完成紀錄。"),
        s("reset","課程設定","#reset","重新開始課程","只清除制度課程進度，不影響案件、表單或系統操作導覽。",{fact:note("影響範圍","只重設副主席制度課程。")})
      ]
    }
  });
})(window);
