...
function answer(selector){return $(selector)?.value.trim()||""}function radioValue(name){return document.querySelector(`input[name="${name}"]:checked`)?.value||""}function mark(value,target){return value===target?"■":"□"}function check(id){return window.FulianInterviewWordDomain.checkboxMark(document.getElementById(id))}function fileDateStamp(d=new Date()){return`${d.getFullYear()}${String(d.getMonth()+1).padStart(2,"0")}${String(d.getDate()).padStart(2,"0")}`}function safeFileName(text){return text.replace(/[\\/:*?"<>|]/g,"-").trim()}function toast(msg){const t=$("#toast");t.textContent=msg;t.classList.add("show");clearTimeout(toast.t);toast.t=setTimeout(()=>t.classList.remove("show"),2200)}
