const $=s=>document.querySelector(s),$$=s=>[...document.querySelectorAll(s)],newMemberTaskId=new URLSearchParams(location.search).get("task");if(!newMemberTaskId)location.replace("case-board.html?new=new");const STORE_KEY=window.FulianCaseDomain.draftStorageKey({id:newMemberTaskId,type:"new"});
const newMemberCompletion=window.FulianInterviewCompletion.setup({formLabel:"新會員訪談"});
const newMemberCalendar=window.FulianCalendarDomain;
let applicants=[{name:"申請者資料載入中"}];
let currentApplicant=applicants[0],saveTimer,newMemberTrainingPicker;
const goalQuestions=[
...
function localDateTime(){return newMemberCalendar.dateTimeInput()}
function localDate(){return newMemberCalendar.dateInput()}
function selectApplicant(name,preserve=false){currentApplicant=applicants.find(x=>x.name===name)||applicants[0];$("#applicantSearch").value=currentApplicant.name;if(!preserve)$("#applicantSignature").value=currentApplicant.name}
function serialize(){const out={trainingSelections:newMemberTrainingPicker?.serialize()||{},applicant:currentApplicant.name,login:$("#loginUser").value};$$('[data-save]').forEach(el=>{if(el.type==="radio"){if(el.checked)out[`radio:${el.name}`]=el.value}else if(el.type==="checkbox")out[el.id]=el.checked;else out[el.id]=el.value});return out}
function restore(data){if(!data)return;if(data.login)$("#loginUser").value=data.login;selectApplicant(data.applicant||applicants[0].name,true);$$('[data-save]').forEach(el=>{if(el.type==="radio")el.checked=data[`radio:${el.name}`]===el.value;else if(el.type==="checkbox"&&data[el.id]!==undefined)el.checked=data[el.id];else if(data[el.id]!==undefined)el.value=data[el.id]});newMemberTrainingPicker?.restore(data.trainingSelections)}
function saveDraft(){localStorage.setItem(STORE_KEY,JSON.stringify(serialize()));$("#saveState").textContent="正在同步 Supabase…";window.FulianCaseStateStore.flush().then(()=>{$("#saveState").textContent="草稿已保存至 Supabase";$("#saveTime").textContent=`最後同步 ${newMemberCalendar.formatTaipeiTime(new Date())}`}).catch(error=>{$("#saveState").textContent=`同步失敗：${error.message}`});updateProgress()}
...
function answer(selector){return $(selector)?.value.trim()||""}function radioValue(name){return document.querySelector(`input[name="${name}"]:checked`)?.value||""}function mark(value,target){return value===target?"■":"□"}function check(id){return window.FulianInterviewWordDomain.checkboxMark(document.getElementById(id))}function fileDateStamp(d=new Date()){return newMemberCalendar.dateStamp(d)}function safeFileName(text){return text.replace(/[\\/:*?"<>|]/g,"-").trim()}function toast(msg){const t=$("#toast");t.textContent=msg;t.classList.add("show");clearTimeout(toast.t);toast.t=setTimeout(()=>t.classList.remove("show"),2200)}
...
async function downloadWord(){
  ...
  const meeting=answer("#meetingDate")?newMemberCalendar.formatTaipeiTimestamp(answer("#meetingDate"),{year:true}):"（未填寫）";
  const meta=new Table({width:{size:100,type:WidthType.PERCENTAGE},borders:noBorders,rows:[new TableRow({children:[metaCell("訪談地址：",answer("#meetingPlace")),metaCell("會談時間：",meeting)]}),new TableRow({children:[metaCell("會員姓名：",currentApplicant.name),metaCell("專業別：",(answer("#profession")||"未填寫")+"（與申請書相同）")]}),new TableRow({children:[metaCell("訪談者：",answer("#leadInterviewer")),metaCell("陪訪：",[answer("#companionInterviewer"),answer("#secondCompanion")].filter(Boolean).join("、"))]})]});
  const children=[new Paragraph({alignment:AlignmentType.RIGHT,children:[run("V8.2　：20251022",{size:18})]}),new Paragraph({alignment:AlignmentType.CENTER,spacing:{after:220},children:[run("高屏區會員訪談表－新會員",{size:32,bold:true,color:"A91419"})]}),meta,
  ...
  const blob=await Packer.toBlob(wordDocument),fileName=`高屏區會員訪談表-新會員-${safeFileName(currentApplicant.name)}-${fileDateStamp()}.docx`;
  try{
    await window.FulianCaseFiles.saveGeneratedWord({caseId:newMemberTaskId,blob,fileName,sourceLabel:"新會員訪談表單",domain:window.FulianCaseDomain,storage:localStorage,indexedDb:indexedDB,FileClass:File});
    newMemberCompletion.success({blob,fileName,caseId:newMemberTaskId,memberName:currentApplicant.name});
    toast("訪談已完成，案件待發送委員回饋");
  }catch(error){
    console.error("案件 Word 保存失敗",error);
    newMemberCompletion.failure({blob,fileName,error});
...
async function lookupCompany(){const taxId=answer("#taxId").replace(/\D/g,""),button=$("#lookupCompany"),status=$("#companyLookupStatus");if(!/^\d{8}$/.test(taxId)){status.textContent="請輸入完整8碼公司統編。";status.className="lookup-status error";return}button.disabled=true;button.textContent="查詢中…";status.textContent="正在查詢經濟部公司登記資料…";status.className="lookup-status";try{const response=await fetch(`/api/company?taxId=${encodeURIComponent(taxId)}`),data=await response.json();if(!response.ok||!data.found)throw new Error(data.message||"查無公司資料");$("#companyName").value=data.name||"";$("#capital").value=data.capital?`${Number(data.capital).toLocaleString("zh-TW")} 元`:"";$("#establishedDate").value=data.setupDate||"";refreshCompanyName();status.textContent=`已帶入公司資料（${data.status||"登記狀態未提供"}），請委員再次核對。`;status.className="lookup-status success";saveDraft()}catch(error){status.textContent=`${error.message}。可改為手動填寫。`;status.className="lookup-status error"}finally{button.disabled=false;button.textContent="查詢公司資料"}}
...
async function init(){await window.FulianCaseStateStore.ready;render();newMemberTrainingPicker=window.FulianTrainingPicker.mount([["mspUpDate","msp_up"],["mspDownDate","msp_down"],["sessionDate1","exchange"],["sessionDate2","exchange"]]);$("#applicantList").innerHTML=applicants.map(x=>`<option value="${x.name}"></option>`).join("");$("#meetingDate").value=localDateTime();$("#signatureDate").value=localDate();$("#leadInterviewer").value=$("#loginUser").value;$("#witness1").value=$("#loginUser").value;$("#chapterNotes").value=window.FulianInterviewWordDomain.DEFAULT_CHAPTER_NOTES;selectApplicant(currentApplicant.name);restore(JSON.parse(localStorage.getItem(STORE_KEY)||"null"));bindRelationFields();refreshCompanyName();$("#witness2").value=answer("#companionInterviewer");$("#witness3").value=answer("#secondCompanion");$("#applicantSearch").onchange=e=>{selectApplicant(e.target.value);saveDraft()};$("#loginUser").onchange=()=>{$("#leadInterviewer").value=$("#loginUser").value;$("#witness1").value=$("#loginUser").value;saveDraft()};$("#resetDraft").onclick=()=>{if(confirm("要清除這份尚未完成的新會員訪談草稿嗎？已保存的案件附件與完成紀錄不會被刪除。")){localStorage.removeItem(STORE_KEY);location.reload()}};$("#downloadWord").onclick=downloadWord;$("#lookupCompany").onclick=lookupCompany;bindInputs();$("#companionInterviewer").addEventListener("input",()=>{$("#witness2").value=answer("#companionInterviewer")});$("#secondCompanion").addEventListener("input",()=>{$("#witness3").value=answer("#secondCompanion")});updateProgress()}
window.FulianNewMemberFormReady=init();
