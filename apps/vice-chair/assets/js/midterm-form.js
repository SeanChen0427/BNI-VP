...
function selectMember(name){member=members.find(x=>x.name===name)||members[0];$("#memberSearch").value=member.name;$("#profession").value=member.profession;rows()}function data(){const o={member:member.name,login:$("#loginUser").value};$$('[data-save]').forEach(e=>o[e.id]=e.value);return o}function save(){localStorage.setItem(KEY,JSON.stringify(data()));$("#saveTime").textContent="正在同步 Supabase…";window.FulianCaseStateStore.flush().then(()=>{const d=new Date();$("#saveTime").textContent=`最後同步 ${d.toLocaleTimeString("zh-TW",{hour:"2-digit",minute:"2-digit"})}`}).catch(error=>{$("#saveTime").textContent=`同步失敗：${error.message}`});progress()}function restore(o){if(!o)return;if(o.login)$("#loginUser").value=o.login;selectMember(o.member);$$('[data-save]').forEach(e=>{if(o[e.id]!==undefined)e.value=["reality_0","forward_2"].includes(e.id)?normalizeScore(o[e.id]):o[e.id]})}function bind(){$$('[data-save]').forEach(e=>e.oninput=()=>{clearTimeout(timer);timer=setTimeout(save,300)})}function progress(){const a=$$('#growSections [data-save], #summary'),n=a.filter(x=>x.value.trim()).length,p=Math.round(n/Math.max(a.length,1)*100);$("#progressBar").style.width=`${p}%`;$("#progressText").textContent=`${p}%`}
