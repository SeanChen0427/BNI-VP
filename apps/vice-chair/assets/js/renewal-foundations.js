(async function () {
  ...
  function openDefinition(item = null, legacy = false) {
    editing = item;
    ...
    $("#sourceTaskId").value = sources.find(source => source.caseId === (item?.caseId || editorContext?.caseId || params.get("case")))?.id || "";
    ...
    for (const [key, value] of [["leadId", item?.leadId], ["companionOne", item?.companionIds?.[0]], ["companionTwo", item?.companionIds?.[1]]]) {
  ...
  $("#definitionForm").onsubmit = event => {
    ...
    save(body, $("#definitionDialog"), $("#definitionError"));
  };
