(function(){
  ...
  function syncCareTask(item){
    ...
    if(!item.owner||!item.dueDate){
      if(existing?.source==="monthly-meeting"&&existing.sourceCareId===item.id){
        tasks=tasks.filter(task=>task.id!==existing.id);localStorage.setItem(TASK_KEY,JSON.stringify(tasks));item.taskId="";item.taskCreatedByMeeting=false;window.dispatchEvent(new CustomEvent("fulian:data-changed",{detail:{source:"monthly-meeting"}}));return"removed";
      }
      return null;
    }
    ...
    localStorage.setItem(TASK_KEY,JSON.stringify(tasks));item.taskId=id;item.taskCreatedByMeeting=task.source==="monthly-meeting";if(item.state==="pending")item.state="scheduled";window.dispatchEvent(new CustomEvent("fulian:data-changed",{detail:{source:"monthly-meeting"}}));return created?"created":"updated";
  }
  ...
  function bind(){
    ...
    $("#finalizeMeeting").onclick=()=>{const missing=missingCareAssignments(record.care?.items||[]);if(missing.length){toast(`尚有 ${missing.length} 項未完成追蹤委員或排定日期，無法結案`);document.querySelector(".care-manage-card.missing-owner")?.scrollIntoView({behavior:"smooth",block:"center"});return}(record.care?.items||[]).forEach(syncCareTask);if(confirm("確認完成本次會員委員會月會並保存為正式紀錄嗎？結案後將改為唯讀。"))save("final")};
