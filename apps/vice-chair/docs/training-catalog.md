...
需求來源：Sean，2026-09-12。部署狀態：2026-09-12 正式課程資料庫、API 與每日同步已啟用；前端發布證據見本次部署紀錄。
...
- 資料表均開啟 RLS 且不授權 anon／authenticated 直接存取；透過 app-api 驗證有效三角色後讀取。`GET /api/training-catalog` 回傳 `fulian.training-catalog.v1`，供本頁及未來表單共用。
- `POST /api/training-catalog`，`{"action":"sync"}`，只允許副主席／Admin。五分鐘共用冷卻與資料庫租約防止重複執行。
- 表單選課關聯、會員異動通知、手動自辦課程與報名／出席紀錄不在本次查詢模組內；既有表單日期與完成記錄維持原流程。
