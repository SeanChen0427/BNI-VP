...
> 本文件的額度、群組人數與投遞數字是對應標示日期的設計證據，不是永久不變的即時平台資訊；正式操作前仍應核對 LINE OA 後台。真正尚未解決的跨系統治理事項集中於 `OPEN_QUESTIONS.md`。
...
| 編號 | 狀態 | 項目與目前處理 |
|---|---|---|
| A1 | 待營運核對 | Webhook 與兩個 Bot 已部署；仍需在 OA 後台定期確認回應模式、Webhook 及自動回應設定未被人工改動。 |
| A2 | 已由架構消除 | `replyToken` 只在同一次 Webhook 立即使用且不落庫，流程不依賴囤積 Token 或精確最長有效秒數。 |
| A3 | 已完成初次拆分；持續治理 | 會員委員秘書Bot已獨立部署並完成投票端到端驗收；現任正式群綁定與換屆核對仍依 `OPEN_QUESTIONS.md` 管理。 |
| A4 | 已確認 | 正式出席公告使用 LINE `textV2` 真正 @所有人；人工複製只是平台異常備援。 |
| A5 | 待營運核對 | `line-reminder-cron` 已部署為正式函式；外部排程的實際呼叫頻率與最近成功紀錄仍須由維運者核對。 |
| A6 | 已確認 | 委員工作進度以正式委員群精確輸入「委員會進度」觸發免費 Reply；工作台 Push 只作明示額度成本的人工備援。 |
| A7 | 待規則確認 | Key-in 提醒的正式最晚送達時點仍需依分會作業確認後調整。 |
| A8 | 待營運核對 | 交流群、公告群與 OA 好友人數會變動，正式額度估算前須讀取／核對當下人數，不得沿用本文件範例固定值。 |
