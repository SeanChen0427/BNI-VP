...
> 2026-08-28 追加決策：會員委員會的案件開票通知不再走一般 Push。副主席從系統複製完整開票文案並貼至委員群，會員委員秘書Bot驗證文案內的一次性呼喚識別與指紋後，以該文字事件的 `replyToken` 免費回覆真正 @所有人通知與投票 Flex 圖卡。月會提醒、工作摘要及案件回饋仍維持既有 Push。
...
- 一般排程提醒的需求判準仍是「副主席不用記、不用搬、不用貼」；案件開票是 2026-08-28 確認的例外，副主席接受在開票當下複製並貼出必要公版，以換取免建 Google 表單及不消耗 Push 額度。
...
每件走完整決策流程的案件（續約／新會員／轉換行業別），2026-08-28 之後的正常路徑為：委員回饋 Push 8 ＋ 委員投票呼喚 Reply 0 ＋ 通過公告 Push 45 ＝ 53 則。開票不再另外扣委員群 8 則。
...
| 委員群（8 人） | 委員會提醒、工作摘要、案件回饋 | **第二個 OA 直接 Push** | 依實際案件數 | 無或二次確認 |
...
- 會員委員會助理（委員群）：以每月 2 案估算為 56／200；案件開票 Reply 為 0 則

### 已否決的方案與理由

| 方案 | 否決理由 |
|---|---|
| 副主席在群組輸入泛用指令觸發排程公告 Reply | 若只是為排程公告人工製造任意訊息，等同自己貼公告，多餘。2026-08-28 新增的案件開票是明確例外：副主席本來就必須發布案件公版，該完整文案同時是投票圖卡的精準呼喚事件。 |
...
### 3.2 為何委員群使用第二個 OA，且案件開票改採呼喚 Reply

委員會人數不隨分會規模成長（副主席 1 ＋ 委員 6 ＋ 董顧 1 ＝ 8），這與分會有 44 人或 100 人無關。

```
委員群 Push 月需求
= 群組人數 × (1 次委員會提醒 + 4 次工作摘要 + 當月案件回饋通知次數)
...
委員群封閉、自然活躍度不確定，因此月會提醒、工作摘要及案件回饋仍不依賴「等待任意群訊息」的機會性投遞，繼續使用獨立 OA 的 Push 額度。

案件開票不同：副主席本來就會在開票當下發布一份完整公版，該則貼文本身可立即提供有效 `replyToken`，不存在等待群組自然訊息的不確定性。2026-08-28 起，開票文案同時帶有一次性呼喚識別與文案指紋；Bot只回覆這個已確認事件，使用 Reply API 回傳真正 @所有人通知與投票 Flex 圖卡，開票通知成本固定為 0 則。
...
案件開票呼喚是上述原則的窄幅例外。Webhook只對會員委員秘書Bot、正式 `committee` 群、文字 `message` 事件暫時讀取內容，解析系統產生的一次性呼喚識別並計算正規化文案指紋；不保存完整文字，也不對一般聊天做關鍵字、語意或會員內容分析。稽核只保存案件、快照、群組、呼喚識別雜湊、文案雜湊、`webhookEventId`、LINE message ID、時間與回覆結果。
...
這不是等待任意群訊息的排程機會性投遞，而是由副主席貼出必要公版時同步觸發。完整公版中的投票網址與圖卡指向同一份投票，但只有 Reply 成功後才收票；若 Reply 失敗，副主席重新貼上同一份完整文案以取得新的 `replyToken`，不得另建第二份投票或自動降級為耗額度的 Push。
