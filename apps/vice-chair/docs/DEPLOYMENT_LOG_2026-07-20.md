...
## 2026-08-05 新會員結案後立即加入點名

- 正式設定頁新增副主席／Admin 專用「新會員登錄」：只接受已完成決策流程並正式結案的新會員案件，保存姓名、專業別、入會確認日及操作者。
- 新增 `provisional_members` 過渡資料層；待 PALMS 會員只加入每週點名與 LINE 公告總人數，不進入續約、期中關懷、會員儀表板或正式分析。
- `attendance_records` 改以正式會員或過渡會員外鍵二擇一保存，前端以 `attendanceId` 識別，支援同名不同專業會員而不互相覆蓋。升格正式會員時，既有點名紀錄由同一交易轉接，不刪除歷史。
- 下一份半年 PALMS 只有在姓名可唯一對帳時才自動升格；同名或既有正式同名時停止自動處理並回報人工確認，PALMS 沒有專業別時不得猜測。
- `20260805090000_provisional_new_members.sql` 已先 dry-run、再正式推送；遠端 migration 清單 local／remote 一致。migration 只有 DDL、權限、trigger 與未來升格 RPC，沒有更新任何既有會員、案件或出席資料。
- `app-api` 已部署為 version 31、狀態 `ACTIVE`、`verify_jwt = true`；GitHub PR #12 squash 合併為 `1fe59e4`，Pages workflow `30994389543` 成功。
- 完整驗證為 72／72 測試通過、專案健檢 0 錯誤、PALMS 46／46 完全吻合；正式 `settings.html` 已確認提供新會員登錄卡與 `settings.js?v=9`。
...
- 分析引擎依「先對帳、後分析」停止產稿：到期報告中的會員代稱03、會員代稱10、會員代稱09、會員代稱02、會員代稱04不在本期 PALMS，也尚未列入正式離會名單。此為資料差異阻擋，不是 API 故障；本輪未代為判斷或修改五人的會員狀態。
...
- 原因確認：同一任務的顯示與表單曾分別信任 URL、月會草稿、任務快取及流程草稿中的姓名；缺少統一的「任務 ID＋案件類型＋會員」核對，舊內容可能把會員代稱05案件顯示成會員代稱06。
...
- 原因確認：五種訪談表的基本初始化與正式案件帶入各自非同步執行；期中表曾先帶入會員代稱05案件，再被較慢完成的預設／草稿初始化覆蓋為會員代稱06，形成會員姓名與主責、陪訪混合。
- 五種訪談表已固定為基本初始化完成後才帶入案件；案件連結同時核對 ID 與類型，正式會員不存在時停止顯示，不再改用第一位會員。
- 正式 GitHub Pages 已部署 commit `d7e41f2`。會員代稱05期中案件首次載入及重新整理後均確認：會員會員代稱05、專業別已核對、主責會員代稱01、陪訪會員代稱08、排定 2026-07-28 19:00，且會員欄位鎖定、無同步警示。
- 後續 commit `186e938` 已將既有案件草稿的會員、會談時間與指派資料依正式任務自動校正並同步回 Supabase；只修補已存在的草稿，不會因開啟空白表單而建立新草稿。
- 透過已連結專案執行唯讀 SQL 核對：`tasks.title` 與 `task_case_states.draft.member` 均為「會員代稱05」，會談時間為 `2026-07-28T19:00`，草稿 revision 為 3。
...
- 正式頁面實測：7 月月會顯示「草稿已保存」，會員代稱07特定關懷為「已排定」；工作台同步顯示「特定關懷・會員代稱07／會員關懷已排定／主責：會員代稱06／原排定 7/22」，確認案件已由 Supabase API 返回。
...
- 會員代稱10、會員代稱09、會員代稱02、會員代稱04及會員代稱03的本期「到期報告有、PALMS 無」差異已完成排除；正式引擎目前回傳 `reconciliation.ok = true`、`issues = []`、現任 43 人。
...
## 2026-08-07 已刪除案件防復活與決議入口修正

- 確認舊裝置的一次性 localStorage 搬移曾用相同案件編號重新匯入已刪除排程；正式套用 `20260805183000_task_delete_tombstones.sql`，以伺服器封存標記阻擋舊裝置復活已刪除案件。
- `app-api` 已重新部署；伺服器找不到案件時仍執行冪等刪除並補上封存標記，舊匯入或延遲 upsert 命中封存案件時改為忽略並回傳正式任務清單。
- 指定舊版特定關懷殘影已完成清除；正式資料庫核對為任務 0 筆、封存標記 1 筆，且該任務沒有正式案件、附件、新會員登錄、回饋或投票資料。
- 案件中心恢復階段式入口：待安排與訪談中不顯示重複流程按鈕；回饋、投票及董顧階段依角色顯示目前可執行的決議工作。
- 完整驗證：72/72 系統測試通過、專案健檢 0 錯誤、PALMS 官方回歸 46/46 完全一致。
