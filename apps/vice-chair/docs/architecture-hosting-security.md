...
狀態：GitHub Pages 正式前台與 Supabase 正式後端均已部署。

首次正式部署確認：2026-07-20（當時完成三角色 Auth／RLS／44 位會員／PALMS 快照線上驗證）；治理更新：2026-09-06

決策來源：Sean 現行決策

## 一、核心決策

- 完整 BNI 分析工具已整合在本總專案的 `../bni-analysis`，工作台透過版本化快照使用其結果，不重寫、不複製第二套計分邏輯。原始 `/Users/chenkuixiang/Desktop/CCHOME/BNI` 只保留為整合前來源與歷史追溯，不再作為修改目標。
...
| 投票資格、票向及時間 | PostgreSQL：`vote_snapshots`、`votes` | 具名保存；副主席／Admin 可查逐人票向，一般委員只可取得自己的票向與整體統計，申請者迴避 |
...
| 委員工作進度發送稽核 | PostgreSQL：Push `committee_work_digest_deliveries`、指令 Reply `committee_work_digest_reply_deliveries` | 只由 Edge Function/service role 讀寫；不保存完整文案、指令、`replyToken` 或 LINE 使用者 ID，只保存來源／文案雜湊、事件識別與發送結果 |
...
- Supabase 是日常正式資料來源；使用者操作不得同時人工維護另一套本機正式資料，以免產生無法判定真假的雙主來源。
- 本機私有分析資料與復原副本是必要資產，必須保留在 Git 之外，並記錄來源期間、匯出時間、schema／應用版本、檔案雜湊及與線上資料的核對結果。
- 只有通過完整性與差異核對的副本才能標示為「可還原」；僅存在於瀏覽器快取、未標版本的檔案或未驗證匯出，不等於可用備份。
- 還原演練必須先在隔離環境進行，不得為了測試直接覆蓋正式 Supabase。完整備份自動化與隔離還原驗證仍列在 `OPEN_QUESTIONS.md`，本文件不宣稱已完成。
- `decision-log.md`、`CHANGELOG.md`、日期化部署紀錄與審查報告是不可改寫的追溯證據；現況文件更新時，應以連結或「已取代」標註保留舊時點，而不是刪除歷史。
...
> 本節保留 2026-07-14 第一版的歷史狀態。其後 Supabase 分析快照、審閱與正式發布流程已完成；目前架構以本文件前段、`ARCHITECTURE_MAP.md` 與部署紀錄為準。
...
- 此版本不寫入 BNI 專案、不解析原始 PALMS，也不重算任何分數；後續已由 Supabase 供應正式快照，並沿用相同 API 契約。
...
- 本機開發／復原預覽使用 AES-256-GCM 加密，將密文與本機主金鑰分別保存於 `~/Library/Application Support/Fulian VP System/`，檔案權限限制為目前電腦使用者。
...
- 正式環境由 Supabase Auth 驗證共用帳號登入，再由 Edge Function 依登入後所選姓名讀取 Edge AES-GCM 加密金鑰；前端只取得綁定狀態與末四碼，不得把完整個人 Key 當成可公開讀取的資料列或一般 Edge Function Secret。
...
`.gitignore`、GitHub Pages 發布前敏感資料掃描及測試資料去識別化規則已啟用；後續每次部署仍必須持續通過，不得因已正式運作而省略。
...
7. 已串接只依系統資料回答的制度查詢 AI，並完成主要 LINE Bot 功能與端到端驗收。
8. 待完成可稽核的本機復原副本、自動匯出／差異核對及隔離還原演練；此項是正式運作後的持續治理，不是尚未上線的功能。

## 九、正式啟用後仍待確認

- 輪替／撤銷 2026-07-20 曾顯示於本機 Codex CLI 輸出的 legacy service role key。
- Supabase 專案由哪個富聯帳號持有、誰付款及誰是第二管理者。
- Supabase 正式方案、資料區域，以及線上備份與本機復原副本的完整範圍、頻率和驗證責任。
...
- LINE 採兩個獨立官方帳號：「副主席秘書Bot」負責公告／董顧／交流群，「會員委員秘書Bot」只負責委員會群。主要功能與投票端到端驗收已完成，兩組 Secret 皆只保存於 Edge Function Secrets；現任正式群綁定的持續核對、兩個 OA 的第二管理者、Token 輪替及換屆交接仍待制度化。
