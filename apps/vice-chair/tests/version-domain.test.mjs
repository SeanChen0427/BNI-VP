...
test("目前 v1.3.3 修正遮罩閃白與表單內逐步教學", () => {
  const validation = validateReleaseTransition(releaseState);
  assert.equal(validation.valid, true);
  assert.equal(releaseState.currentVersion, "1.3.3");
  assert.equal(releaseState.previousVersion, "1.3.2");
...
test("版本預覽指令會從目前正式版本計算下一版", () => {
  ...
  assert.match(result.stdout, /目前：v1\.3\.3/);
