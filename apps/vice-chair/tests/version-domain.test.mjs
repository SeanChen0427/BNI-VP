...
test("目前 v1.7.0 提供夥伴多選與名單匯出", () => {
  const validation = validateReleaseTransition(releaseState);
  assert.equal(validation.valid, true);
  assert.equal(releaseState.currentVersion, "1.7.0");
  assert.equal(releaseState.previousVersion, "1.6.0");
...
test("版本預覽指令會從目前正式版本計算下一版", () => {
  ...
  assert.match(result.stdout, /目前：v1\.7\.0/);
  assert.match(result.stdout, /分類：feature → minor/);
  assert.match(result.stdout, /下一版：v1\.8\.0/);
});
