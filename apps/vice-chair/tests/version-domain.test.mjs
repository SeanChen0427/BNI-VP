...
test("目前 v1.2.0 是從 v1.1.3 增加地基追蹤的功能版", () => {
  const validation = validateReleaseTransition(releaseState);
  assert.equal(validation.valid, true);
  assert.equal(releaseState.currentVersion, "1.2.0");
  assert.equal(releaseState.previousVersion, "1.1.3");
  assert.equal(releaseState.changeType, "feature");
});
...
test("版本預覽指令會從目前正式版本計算下一版", () => {
  ...
  assert.match(result.stdout, /目前：v1\.2\.0/);
  assert.match(result.stdout, /分類：feature → minor/);
  assert.match(result.stdout, /下一版：v1\.3\.0/);
});
