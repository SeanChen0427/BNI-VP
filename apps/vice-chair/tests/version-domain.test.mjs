...
test("目前 v1.7.4 擴充會員互動歷程", () => {
  const validation = validateReleaseTransition(releaseState);
  assert.equal(validation.valid, true);
  assert.equal(releaseState.currentVersion, "1.7.4");
  assert.equal(releaseState.previousVersion, "1.7.3");
...
test("版本預覽指令會從目前正式版本計算下一版", () => {
  ...
  assert.match(result.stdout, /目前：v1\.7\.4/);
