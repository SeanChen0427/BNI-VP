...
test("目前 v1.3.0 新增多元地基與會員分組", () => {
  const validation = validateReleaseTransition(releaseState);
  assert.equal(validation.valid, true);
  assert.equal(releaseState.currentVersion, "1.3.0");
  assert.equal(releaseState.previousVersion, "1.2.1");
...
test("版本預覽指令會從目前正式版本計算下一版", () => {
  ...
  assert.match(result.stdout, /目前：v1\.3\.0/);
  assert.match(result.stdout, /分類：feature → minor/);
  assert.match(result.stdout, /下一版：v1\.4\.0/);
});
