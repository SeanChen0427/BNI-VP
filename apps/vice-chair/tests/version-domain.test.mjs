...
test("目前 v1.2.1 是簡化地基設定的修正版", () => {
  const validation = validateReleaseTransition(releaseState);
  assert.equal(validation.valid, true);
  assert.equal(releaseState.currentVersion, "1.2.1");
  assert.equal(releaseState.previousVersion, "1.2.0");
...
test("版本預覽指令會從目前正式版本計算下一版", () => {
  ...
  assert.match(result.stdout, /目前：v1\.2\.1/);
