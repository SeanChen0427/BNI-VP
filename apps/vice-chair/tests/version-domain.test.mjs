...
test("目前 v1.5.0 全面整合副主席交接教學", () => {
  const validation = validateReleaseTransition(releaseState);
  assert.equal(validation.valid, true);
  assert.equal(releaseState.currentVersion, "1.5.0");
  assert.equal(releaseState.previousVersion, "1.4.0");
...
test("版本預覽指令會從目前正式版本計算下一版", () => {
  ...
  assert.match(result.stdout, /目前：v1\.5\.0/);
  assert.match(result.stdout, /分類：feature → minor/);
  assert.match(result.stdout, /下一版：v1\.6\.0/);
});
