...
test("目前 v1.1.2 是從 v1.1.1 發布的相容修正版", () => {
  const validation = validateReleaseTransition(releaseState);
  assert.equal(validation.valid, true);
  assert.equal(releaseState.currentVersion, "1.1.2");
  assert.equal(releaseState.previousVersion, "1.1.1");
...
test("版本預覽指令會從目前正式版本計算下一版", () => {
  ...
  assert.match(result.stdout, /目前：v1\.1\.2/);
