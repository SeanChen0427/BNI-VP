...
test("目前 v1.8.3 修正未完成工作刪除", () => {
  const validation = validateReleaseTransition(releaseState);
  assert.equal(validation.valid, true);
  assert.equal(releaseState.currentVersion, "1.8.3");
  assert.equal(releaseState.previousVersion, "1.8.2");
...
test("版本預覽指令會從目前正式版本計算下一版", () => {
  ...
  assert.match(result.stdout, /目前：v1\.8\.3/);
